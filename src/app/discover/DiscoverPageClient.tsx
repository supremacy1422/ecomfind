...
export default function DiscoverPageClient() {
  ...
  const [recentAudits, setRecentAudits] = useState<string[]>(["fashionnova.com", "gymshark.com", "allbirds.com"]);
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem("ecomfind_recent_audits");
    if (stored) {
      try { setRecentAudits(JSON.parse(stored)); } catch {}
    }
  ...
  const runAudit = async (overrideUrl?: string) => {
    ...
    const domain = fullUrl.replace(/^https?:\/\//, "").split("/")[0].replace(/^www\./, "");
    ...
    try {
      ...
      setRecentAudits((prev) => {
        const next = [domain, ...prev.filter((x) => x !== domain)].slice(0, 5);
        localStorage.setItem("ecomfind_recent_audits", JSON.stringify(next));
        return next;
      });
    } catch (err: any) {
      console.error(err);
  ...
  const addToLeads = () => {
    if (!data) return;
    const lead = { email: data.emails[0] || "", name: data.domain, store: data.domain, url: `https://${data.domain}` };
    const existing = JSON.parse(localStorage.getItem("ecomfind_bulk_leads") || "[]");
    localStorage.setItem("ecomfind_bulk_leads", JSON.stringify([...existing, lead]));
    alert(`Added ${data.domain} to your leads!`);
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-16 min-h-[60vh]">
            <div>
              ...
              >
                ...
                >
                  {loading ? (
                    <><IconClock className="w-4 h-4 animate-spin" /> Auditing...</>
                  ) : (
                    <><IconSearch className="w-4 h-4" /> Run AI Revenue Audit</>
                  )}
              ...
              {recentAudits.length > 0 && (
                <div className="flex items-center gap-2 mb-6 flex-wrap">
                  ...
                  {recentAudits.map((r) => (
                    <button key={r} onClick={() => { setUrl(r); runAudit(r); }} className="text-[11px] px-2 py-1 rounded-full bg-slate-800/60 border border-slate-700/40 text-slate-400 hover:text-white hover:border-slate-600 transition-colors">
                      {r}
                    </button>
                  ))}
                </div>
          ...
          <div ref={reportRef} className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs text-slate-500 mr-2">Auditing: {data.domain}</span>
                <button onClick={addToLeads} className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 text-xs font-semibold rounded-lg transition-colors flex items-center gap-1.5">
                  <IconPlus className="w-3.5 h-3.5" /> Add to Leads
                </button>
              </div>
            </div>
            ...
            <SectionCard title="Discovered Emails & Quick Outreach" icon={IconMail}>
              <div className="space-y-3">
                ...
                {outreachStatus && (
                  <p className={`text-sm text-center ${outreachStatus.includes("✓") ? "text-emerald-400" : "text-rose-400"}`}>
                    {outreachStatus}
                  </p>
                )}
              </div>
