  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (inQuotes) {
      ...
      if (char === '"') {
        ...
        row.push(cell.trim());
        if (row.length > 0 && row.some(c => c.length > 0)) result.push(row);
        row = [];
        cell = "";
      } else if (char === '\r') {
        // skip
...
export default function LeadsPage() {
  ...
  const filteredLeads = useMemo(() => {
    ...
    if (activeQuickFilter) {
      result = result.filter(l => l.industry?.toLowerCase().includes(activeQuickFilter.toLowerCase()));
    }

    if (nicheFilter !== "all") {
      result = result.filter(l => l.industry?.toLowerCase().includes(nicheFilter.toLowerCase()));
    }
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [search, activeQuickFilter, nicheFilter]);

  /* ─── Stats ─── */
  const stats = useMemo(() => {
    const total = leads.length;
    const validEmails = leads.filter(l => l.email && l.email.length > 3).length;
    const highQuality = leads.filter(l => (l.quality_score || 0) >= 70).length;
    const uncontacted = leads.filter(l => !l.status || l.status === "new").length;
    return { total, validEmails, highQuality, uncontacted };
  }, [leads]);
  ...
  const parseAndImport = async (text: string) => {
    ...
    const domainIdx = getCol(["domain", "store_name", "store name", "name", "url", "website", "website url", "site"]);
    const emailIdx = getCol(["email", "e-mail", "contact_email", "contact email"]);
    const firstNameIdx = getCol(["first_name", "first name", "firstname", "fname"]);
    const lastNameIdx = getCol(["last_name", "last name", "lastname", "lname"]);
    const countryIdx = getCol(["country", "location", "region", "geo"]);
    const industryIdx = getCol(["industry", "niche", "category", "vertical", "sector"]);
    const companySizeIdx = getCol(["company_size", "company size", "size", "employees"]);
    const revenueRangeIdx = getCol(["revenue_range", "revenue range", "revenue", "annual revenue"]);
    const qualityScoreIdx = getCol(["quality_score", "quality score", "score", "lead_score"]);

    if (domainIdx === -1 && emailIdx === -1) {
      setImportError("Could not find a recognizable column. Expected: domain, store_name, name, url, website, or email.");
      return;
    }
    ...
    for (let i = 1; i < parsed.length; i++) {
      ...
      if (firstNameIdx !== -1 && vals[firstNameIdx]) row.first_name = vals[firstNameIdx].trim();
      if (lastNameIdx !== -1 && vals[lastNameIdx]) row.last_name = vals[lastNameIdx].trim();
      if (countryIdx !== -1 && vals[countryIdx]) row.country = vals[countryIdx].trim();
      if (industryIdx !== -1 && vals[industryIdx]) row.industry = vals[industryIdx].trim();
      if (companySizeIdx !== -1 && vals[companySizeIdx]) row.company_size = vals[companySizeIdx].trim();
      if (revenueRangeIdx !== -1 && vals[revenueRangeIdx]) row.revenue_range = vals[revenueRangeIdx].trim();
      if (qualityScoreIdx !== -1 && vals[qualityScoreIdx]) {
        const qs = parseFloat(vals[qualityScoreIdx]);
        if (!isNaN(qs)) row.quality_score = Math.round(qs);
      }
    ...
    if (rows.length === 0) {
      setImportError("No valid rows found. Check your column headers.");
      return;
    }
    ...
    <div className="min-h-screen bg-[#0b0f1f] text-slate-200">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-col lg:flex-row gap-4 mb-6">
          <div className="flex-1 flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search by domain, email, country, or niche..."
            ...
            <select
              ...
              className="px-4 py-2.5 bg-slate-900/60 border border-slate-700 rounded-xl text-sm text-slate-300 focus:outline-none focus:ring-2 focus:ring-violet-500/40"
            >
            ...
            <button
              onClick={() => { setSearch(""); setNicheFilter("all"); setActiveQuickFilter(null); }}
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-4 py-2.5 bg-violet-600 hover:bg-violet-500 text-white font-semibold rounded-xl text-sm transition-all flex items-center gap-2"
            >
            ...
            <button
              ...
              className="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-xl text-sm font-medium transition-colors flex items-center gap-2 disabled:opacity-50"
            >
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          ...
          <button
            onClick={() => setShowImport(!showImport)}
            className="px-3 py-1.5 rounded-lg text-xs font-medium border bg-slate-900/60 text-slate-400 border-slate-700 hover:border-slate-600 transition-all flex items-center gap-1"
          >
            <IconUpload className="w-3 h-3" /> Import CSV
          </button>
        </div>
          ...
          <div className="rounded-2xl bg-slate-900/60 border border-slate-800 p-6 mb-8">
            <div className="flex items-start gap-3 mb-4 p-3 rounded-lg bg-amber-500/5 border border-amber-500/10">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Upload CSV files only. Supported columns: domain, email, first_name, last_name, country, industry, company_size, revenue_range, quality_score.</p>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-lg text-sm text-slate-300 file:mr-4 file:py-1 file:px-3 file:rounded-md file:border-0 file:bg-violet-600 file:text-white file:text-xs file:font-semibold hover:file:bg-violet-500"
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="domain,email,first_name..."
                  className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-lg text-sm text-slate-100 placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-violet-500/50 resize-none mb-2"
                />
                <button
                  onClick={handlePasteImport}
                  className="px-4 py-2 bg-violet-600 hover:bg-violet-500 text-white font-semibold rounded-lg text-xs transition-all"
                >
                  Import Pasted Data
                </button>
              </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {selected.size > 0 && (
              <button
                ...
                className="px-3 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 text-rose-400 rounded-lg text-xs font-medium transition-colors flex items-center gap-1.5"
              >
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-slate-600">
            ...
            <p className="text-xs">Import a CSV or search StoreIndex to get started.</p>
          </div>
        ) : (
            ...
            {paginatedLeads.map((lead) => (
              ...
              >
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      className="w-4 h-4 rounded border-slate-600 bg-slate-900 text-violet-500 focus:ring-violet-500/50"
                    />
                    <div>
                      <a
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-full border font-semibold ${qualityColor(lead.quality_score)}`}>
                    {qualityLabel(lead.quality_score)}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2 pt-3 border-t border-slate-800">
                  {lead.email ? (
                    <a
                      href={`/outreach?email=${encodeURIComponent(lead.email)}&domain=${encodeURIComponent(lead.domain)}`}
                    ...
                    <button
                      onClick={() => findEmailOSINT(lead)}
                      disabled={osintLoading === lead.id}
                      className="flex-1 px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-xs font-medium transition-colors flex items-center justify-center gap-1.5 border border-slate-700"
                    >
                      {osintLoading === lead.id ? (
                        <span className="animate-spin w-3 h-3 border-2 border-slate-500 border-t-transparent rounded-full" />
                      ) : (
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2">
            <button
              ...
              className="p-2 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 disabled:opacity-40 transition-colors"
            >
